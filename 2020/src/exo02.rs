...
pub fn count_a(v: Vec<&str>) -> usize {
    v.iter().filter(|s| check_a(s)).count()
}
pub fn count_b(v: Vec<&str>) -> usize {
    v.iter().filter(|s| check_b(s)).count()
}

pub fn check_a(s: &str) -> bool {
    let (min, max, letter, password) = parse(s);
    let mut count = 0;
...
pub fn check_b(s: &str) -> bool {
    let (min, max, letter, password) = parse(s);
    let first = min - 1;
    let second = max - 1 - (first + 1);
    let mut password = password.chars();
    let first = password.nth(first as usize).unwrap();
    let second = password.nth(second as usize).unwrap();

    (first == letter || second == letter) && first != second
}

fn parse(s: &str) -> (i32, i32, char, String) {
    let re = Regex::new(r"([0-9]+)-([0-9]+) ([a-z]): ([a-z]+)").unwrap();
    for cap in re.captures_iter(s) {
        // println!("Month: {} Day: {} Year: {}", &cap[2], &cap[3], &cap[1]);
        let min = i32::from_str_radix(&cap[1], 10).unwrap();
        let max = i32::from_str_radix(&cap[2], 10).unwrap();
...
mod test {
    ...
    #[test]
    fn part_a_samples() {
        assert_eq!(check_a("1-3 a: abcde"), true);
        assert_eq!(check_a("1-3 b: cdefg"), false);
        assert_eq!(check_a("1-9 c: ccccccccc"), true);
    }
    #[test]
    fn part_a_count() {
        assert_eq!(
            count_a(vec!["1-3 a: abcde", "1-3 b: cdefg", "1-9 c: ccccccccc"]),
            2
        );
    }
    #[test]
    fn part_b_samples() {
        assert_eq!(check_b("1-3 a: abcde"), true);
        assert_eq!(check_b("1-3 b: cdefg"), false);
        assert_eq!(check_b("1-9 c: ccccccccc"), false);
        assert_eq!(check_b("1-3 a: cbade"), true);
    }
    #[test]
    fn part_b_count() {
        assert_eq!(
            count_b(vec!["1-3 a: abcde", "1-3 b: cdefg", "1-9 c: ccccccccc"]),
            1
        );
    }
